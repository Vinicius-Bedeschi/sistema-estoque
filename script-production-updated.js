...
async function handleLogin(e) {
    ...
    try {
        ...
        if (error.message.includes('Failed to fetch')) {
            showError('Erro de conexão: Verifique se o Google Apps Script está implantado corretamente e se a URL está correta. Erro: ' + error.message);
        } else {
...
function setupEventListeners() {
    ...
    // Fechar modal clicando fora (exceto modal de login)
    ...
    // Prevenir bypass de login - verificar autenticação em todas as ações
    document.addEventListener('click', (e) => {
        // Se não há usuário logado e não é uma ação de login, bloquear
        if (!AppState.currentUser && !e.target.closest('#login-modal') && !e.target.closest('#login-form')) {
            const interactiveElements = ['button', 'a', 'input[type="submit"]', '.tab-btn', '.btn'];
            if (interactiveElements.some(selector => e.target.matches(selector) || e.target.closest(selector))) {
                e.preventDefault();
                e.stopPropagation();
                showLoginModal();
                showError('Você precisa fazer login para acessar esta funcionalidade.');
                return false;
            }
        }
    });
...
function initializeApp() {
    ...
    // Sempre esconder o conteúdo principal inicialmente
    hideMainContent();
    ...
    if (savedUser) {
        try {
            ...
            // Validar se o usuário salvo tem as propriedades necessárias
            if (user && user.username && user.perfil) {
                AppState.currentUser = user;
                showMainContent();
                loadInitialData();
            } else {
                // Usuário inválido, forçar novo login
                localStorage.removeItem('currentUser');
                AppState.currentUser = null;
                showLoginModal();
            }
 ...
 * 
 * A marca d'água "Criado por: Vinícius Bedeschi ® - v 1.1 - 2025" está localizada em:
 * 
